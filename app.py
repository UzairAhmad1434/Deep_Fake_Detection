...
<style>
    ...
    .logo-img {
        ...
        margin-top: 15px; /* Increased top margin to the logo */
    }
    ...
    .logo-container {
        ...
        justify-content: center;
        align-items: center;
        padding-top: 20px; /* Increased padding to the top */
        height: 100%;
    }
...
def load_logo():
    ...
    try:
        ...
        # Resize to appropriate dimensions for the header
        return img.resize((100, 100))
    except Exception as e:
        st.error(f"Error loading logo image: {str(e)}")
        # Create a fallback SVG in case the image file is not found
        svg_code = """
        ...
        """
        return svg_code, True  # Second value indicates this is SVG
...
def page_1():
    ...
    col1, col2 = st.columns([0.6, 3])  # Adjusted ratio for better spacing
    with col1:
        # Display the logo with additional padding at top
        if st.session_state.is_svg:
            ...
            st.markdown('</div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div style="padding-top: 0.8rem;">', unsafe_allow_html=True)  # Increased padding-top
    ...
    if uploaded_file is not None:
        ...
        # Display uploaded image (resized to smaller dimensions)
        img = Image.open(uploaded_file)
        img = img.resize((300, 300))  # Resize to a smaller image
                ...
                with st.spinner("Analyzing image patterns..."):
                    try:
                        ...
                        if img_array is not None:
                            ...
                            # Prediction logic: >0.5 is Real, ≤0.5 is AI-Generated
                            ...
                            st.session_state.page = 2  # Move to results page
                    except Exception as e:
...
def page_2():
    # Header section with improved spacing (same as page_1)
    st.markdown('<div class="header" style="display: flex; align-items: center; gap: 30px; padding: 1.5rem 0;">', unsafe_allow_html=True)
    col1, col2 = st.columns([0.6, 3])  # Adjusted ratio for better spacing
    with col1:
        # Display the logo with additional padding at top
    ...
    with col_img:
        if st.session_state.uploaded_file is not None:
            ...
            img = img.resize((250, 250))  # Slightly smaller image for better fit
            st.image(img, caption="Uploaded Image", use_container_width=False)
    ...
    # Navigation buttons - more compact layout
    btn_cols = st.columns([1, 1, 2])  # Added an empty column for spacing
    with btn_cols[0]:
